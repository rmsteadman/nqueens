 ...
 */
    ...
    hasRowConflictAt: function(rowIndex) {
      ...
      // increment count if duplicate pieces are found
        if (board[rowIndex][i] === 1){
          count++;
        }
      }

      // return true if conflict exist, false otherwise
      if (count <= 1){
        return false;
      } else {
        return true;
    ...
    hasMajorDiagonalConflictAt: function(majorDiagonalColumnIndexAtFirstRow) {
      ...
      for (var i = 0; i < board.length; i++){
        /*check to see if the value is 1, if it is, increment count*/
        if (board[i][j] === 1) {
          count++;
        }
        j++;
    ...
    hasAnyMinorDiagonalConflicts: function() {
      ...
      for (var i = ((board.length - 1) * 2); i >= 0; i--){
